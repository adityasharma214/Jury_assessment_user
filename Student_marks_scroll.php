    ...
    <section class="section">
        ...
        <div class="container mt-4">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    while ($rows = $result->fetch_assoc()) {
                        $count += $count;
                        $rubrics =  $rows['Rubrics_id'];
                    ?>
                        <tr>
                            <td hidden><input type="text" name="rubrics[]" value="<?php echo $rows['Rubrics_id']; ?> "></td>
                            <td><?php echo $rows['Criteria']; ?></td>
                            <td><?php echo str_replace("\n","<br />", $rows['Rating1']); ?></td>
                            <td><?php echo str_replace("\n","<br />", $rows['Rating2']); ?></td>
                            <td><?php echo str_replace("\n","<br />", $rows['Rating3']); ?></td>
                            <td style="vertical-align:middle; text-align:center;"><input required min="0" max="<?php echo $rows['Out of']; ?>" step="1" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" type="number" maxlength="2" name="data[]" /></td>
            ...
            <div class="form-group">
                ...
                <textarea style="margin-bottom: 20px;" name="comments[]" class="form-control" id="exampleFormControlTextarea1" rows="3" maxlength="250" placeholder="Enter your Feedback"><?php echo $data["Subject_id"] . ' - ' . $data["Subject_name"] . ' - '; ?></textarea>
            </div>
